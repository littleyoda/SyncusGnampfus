...
final class InterestApiClient {
  ...
  private static final String GRAPHQL_URL = "https://de.scalable.capital/interest/api/graphql/";
  private static final String REFERER = "https://de.scalable.capital/interest/%s/";
  private static final String USER_AGENT = "scalable-capital-java/0.1.0";

  GraphqlEnvelope execute(Session session, String savingsAccountId, GraphqlRequestBody requestBody)
    ...
    HttpRequest request = HttpRequest.newBuilder(URI.create(GRAPHQL_URL))
        ...
        .header("Referer", REFERER.formatted(savingsAccountId))
    ...
    if (response.statusCode() < 200 || response.statusCode() >= 300) {
      throw new IOException("Interest GraphQL request failed: " + response.statusCode() + " " + response.body());
    }
    ...
    if (envelope.errors != null && !envelope.errors.isEmpty()) {
      throw new IOException("Interest GraphQL returned errors: " + toPrettyJson(envelope.errors));
    }
    return envelope;
  }
